...
using PlutoUI # This is for sliders
...
# ╔═╡ 56640c41-fbab-47b2-9473-87f8c0fd80cb
begin 
	using DataFrames
	using XLSX, CSV
end
...
# ╔═╡ a508af1c-82d7-4294-a1ea-84dccf0b3906
function random(arg1 = 1)
	if arg1 == 1
		return rand() * 2 - 1 
	end
	return rand(arg1) * 2 - (ones(arg1))
end

# ╔═╡ 6b666280-7794-46af-9a73-597f4ae24a55
random() # use this cell to implement your solution

# ╔═╡ b5b29882-e97d-4888-b7f0-9a66f5e6cbf5
random(3)
...
x_random = random(n_uniform) # use this cell and as many cell as you need

# ╔═╡ 6b44b261-bd09-4a5d-b602-fb1a77b4f320
scatter(1:n_uniform, x_random)

# ╔═╡ 647fa848-9bd9-46a4-86aa-bc1ddd8dffa8
histogram(x_random, bins = 50)
...
function randMatrix(arg1, arg2)
	return rand(arg1, arg2) * 6 - ones(arg1, arg2) * 3
end

# ╔═╡ fb355c9c-3e67-453c-96a4-eafef1519427
randMatrix(3, 4)
...
# ╔═╡ f18d8736-edfd-419c-a72d-2da94fa7ceca
md"
μ = $(@bind mean Slider(-2:2, show_value=true))

σ = $(@bind sigma Slider(1:5, show_value=true))
"

# ╔═╡ 6313b2c0-6a6d-477a-afe1-a67389c8b3f0
# use this cell and as many cell as you need
x_vals = randn(n_normal) * sigma + mean * ones(n_normal)
...
# ╔═╡ ee708169-5aa5-498c-9f89-8b0373f47e2e
	histogram(x_vals, bin = 100, norm = true, fillalpha=0.4, xlims = (-15,15),label = "Normalized histogram", fg_legend = false)# (-3σ,3σ))
...
md"""
A seed is used to initialize a pseudorandom number generator, making it possible to reproduce the same sequence of random numbers in each execution of the cell. 
"""
...
md"""
To generate normally distributed data with a given mean (μ) and standard deviation (σ) from the randn() function, the following steps can be applied: 

randn() generates random numbers from a standard normal distribution with a mean of 0 and a standard deviation of 1.

To obtain data with the desired mean (μ) and standard deviation (σ), there is a need to scale and shift the random numbers generated by randn().

Multiplication of the data generated by the randn() by the desired standard deviation (σ), scales the data to have the desired spread.

Addition of the the desired mean (μ) to the scaled value shifts the data to the desired mean.

Eventually, this will give normally distributed data with the specified mean (μ) and standard deviation (σ).
"""
...
# use this cell 
md"""
To transform normally distributed data with a given mean (μ) other than zero and standard deviation (σ) other than one, to a standard normal distribution data the following steps can be applied: 

Substraction of the the mean of the data from each data entry shifts the data to the mean of zero. 

Division of the standard deviation of the data, scales down the data to obtain standard normal distribution. 

This transformation is also known as standardizing the data. 
"""
...
# ╔═╡ f98515a0-b8a7-4429-b85b-bccc22fdd70b
generate()
...
function sumArray(v1)
	return v1'*ones(length(v1))
end

# ╔═╡ 53a47452-135a-4fe0-85aa-9ca2aa3a9fce
v1 = [x for x in 1:6]

# ╔═╡ bf444ead-c9fb-4437-9335-b4b5a1cf8f06
sumArray(v1)

# ╔═╡ 4998c8b7-b701-4bf8-96fa-9bfc277aed79
function averageArray(v1)
	len = length(v1)
	return v1'*(ones(len) / len)
end

# ╔═╡ 9e93f728-bf65-4f1f-a459-2aa71f4d8961
averageArray(v1)

# ╔═╡ a548e581-08b0-47be-aa70-36dff7d60012
pwd()

# ╔═╡ dcf78f96-cf22-4c0b-ad4b-325184dc42b3
df_house = CSV.read("kc_house_data.csv", DataFrame)

# ╔═╡ 14a22997-403b-4d23-ae82-06b185679157
averageArray(df_house.price)

# ╔═╡ 3bef107b-2bfd-4f95-ab5a-aa1653c816b3
maximum(df_house.price)
...
!!! hint
	...
	##### Don't forget that input is taken as string. So, use parse(Int, "Input") you can convert string to number.
...
begin
	...
	guess_num = parse(Int, input_c)
	if (guess_num == target_num)
		println("Guess is correct! You won the game.")
	elseif (guess_num > target_num)
		println("Guess is higher than the target! Make a new guess.")
	else
		println("Guess is lower than the target! Make a new guess.")
	end
end
...
# ╔═╡ aff7c45b-0ee5-4c8e-a6bc-fe8c25961cf6
basic_square(4)
...
# ╔═╡ a30bd2f5-7ab4-4acc-ba43-c6033ae59ab9
square(4)
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "0f328dab4cf848771997d6847c5bdb775a45d8e6"
